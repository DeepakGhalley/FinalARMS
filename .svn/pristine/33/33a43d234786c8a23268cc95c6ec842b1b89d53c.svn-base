CREATE PROCEDURE [dbo].[GetHouseRentDemandScheduleMonthly]
...
(
 ...
 SELECT  ROW_NUMBER() OVER(
  ...
  - CASE WHEN @e < ld THEN (DATEDIFF(DAY, @e, ld)-1) ELSE 0 END
   ,(select (demandNoId) from tblHouseRentDemandDetail where houseAllocationId=@Id and demandYear=DATENAME(year, fd) and demandYear=@yr and demandGenerateScheduleId= MONTH(fd)) as DemandNoId
