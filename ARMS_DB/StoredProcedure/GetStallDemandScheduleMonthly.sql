CREATE PROCEDURE [dbo].[GetStallDemandScheduleMonthly]
...
(
 ...
 SELECT  ROW_NUMBER() OVER(
  ...
  - CASE WHEN @e < ld THEN (DATEDIFF(DAY, @e, ld)-1) ELSE 0 END
    ...
    ,(select (demandNoId) from tblStallDetailDemand where stallAllocationId=@Id and demandYear=DATENAME(year, fd)and demandGenerateScheduleId= MONTH(fd)) as DemandNoId
	 ,TotalDays=day(EOMONTH((fd))),fd
